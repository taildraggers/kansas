"""Aggregates Kansas-located listings from every taildraggers.com
...
ones whose location ends in ", KS".
...
Indiana/Iowa repos' approach, with the state filter swapped to "KS".
"""
...
PAGE_TITLE = "Taildragger Ads in Kansas"
STATE_ABBR = "KS"
...
_KANSAS_LOCATION_RE = re.compile(r",\s*KS\s*$", re.IGNORECASE)


def _is_kansas(location: str) -> bool:
    return bool(_KANSAS_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        kansas_rows = [row for row in rows if _is_kansas(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(kansas_rows)} in Kansas")
        all_listings.extend(kansas_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Kansas listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Kansas listings: {len(listings)}")
