...
Daily aggregator of Kansas-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "KS".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, KS` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Kansas"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/kansas/`
...
```html
<iframe
  src="https://taildraggers.github.io/kansas/"
  title="Taildragger Ads in Kansas"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_KANSAS_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
